...
def masuk():
    ...
    mbo = raw_input('\n%s [?] Menu : %s'%(P,H))
    if mbo in(""):
    	print("%s [!] Isi yang benar Ya Kakang/Teteh "%(M));exit()
    elif mbo in ('1','01'):
        jalan("\n%s [%s!%s] Wajib gunakan akun baru/tumbal dilarang akun utama"%(P,M,P))
    	romz = raw_input('%s [?] Token : %s'%(P,H))
        if romz in(""):
        	print("%s [!] Isi yang benar Hay manusia "%(M));exit()
    	try:
